...
const TransactionSigningHashKey = createPaddedBuffer("TransactionSigningHash");
const TransactionHashDomain = createPaddedBuffer("TransactionHash");

const NewTransactionSigningHashWriter = (buf, key) => {
	let hash = NobleHashing.blake3.create({ dkLen: 32, key: key, });
...
// NewtransactionHashWriter
class NewTransactionHashWriter {
	constructor() {
    // Initialize a new BLAKE3 hasher
		this.hash = NobleHashing.blake3.create({ dkLen: 32, key: TransactionHashDomain })
...
NewTransactionHashWriter.prototype.writeUInt8 = function(value){
	const buf = new BufferWriter();
	buf.writeUInt8(value);
	this.hash.update(buf.toBuffer());
}

NewTransactionHashWriter.prototype.writeUInt16LE = function(value){
	const buf = new BufferWriter();
	buf.writeUInt16LE(value);
	this.hash.update(buf.toBuffer());
}

NewTransactionHashWriter.prototype.writeUInt32LE = function(value){
	const buf = new BufferWriter();
	buf.writeUInt32LE(value);
	this.hash.update(buf.toBuffer());
}

NewTransactionHashWriter.prototype.writeUInt64LE = function(value){
	const buf = new BufferWriter();
	buf.writeUInt64LEBN(BN.fromNumber(value));
	this.hash.update(buf.toBuffer());
}

NewTransactionHashWriter.prototype.writeVarBytes = function(buf){
	this.writeUInt64LE(buf.length);
	this.hash.update(buf);
}

NewTransactionHashWriter.prototype.writeHash = function(buf){
	this.hash.update(buf);
}

NewTransactionHashWriter.prototype.finalize = function(){
	return this.hash.digest();
}
...
function getPreviousOutputsHash(transaction, sighashType, reusedValues){
	...
	if (_.isUndefined(reusedValues.previousOutputsHash)){
		const hashWriter = new NewTransactionHashWriter();
...
/**
 * @param {NewTransactionHashWriter} hashWriter
 * @param {Input} input
 *
 */
...
var sighash2 = function sighash(transaction, sighashType, inputNumber, subscript, satoshisBN, flags) {
	...
	let buf = new BufferWriter()
		...
		.toBuffer()
	let ret = NewTransactionSigningHashWriter(buf, TransactionSigningHashKey);
