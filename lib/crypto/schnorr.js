...
Schnorr.prototype._findSignature = function(d, e) {
  ...
  Schnorr.prototype.nonceFunctionRFC6979 = function(privkey, msgbuf) {
    ...
    K = Hash.blake3hmac(Buffer.concat([V, Buffer.from('00', 'hex'), blob]), K);
    V = Hash.blake3hmac(V,K); 

    K = Hash.blake3hmac(Buffer.concat([V,Buffer.from('01','hex'), blob]), K);
    V = Hash.blake3hmac(V,K);
    ...
    while (true) {
      ...
      K = Hash.blake3hmac(Buffer.concat([V, Buffer.from("00", 'hex')]), K);
